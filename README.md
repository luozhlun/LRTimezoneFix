...
- ExifTool 全程在后台静默运行，扫描最多使用 4 个独立常驻会话；修复的写入与写后读取复用同一个常驻会话。
...
- 压缩扫描结果标题与统计卡片，为文件明细留出更多空间。
- 移除结果列表上方的 XMP 提示横幅，将说明放在 XMP 文件详情中；修复完成后仍提示同步 Lightroom 目录。
- 文件读取进度恢复为 0–100%，后续时区分析单独显示状态。

### 2.1.0

- 扫描按文件分配给最多 4 个 ExifTool 常驻会话并行读取，进度统一汇总，取消后等待会话退出。
- JPG 扫描不再计算图像数据哈希；仅对实际选中修复的 JPG 在写入前计算，写后仍验证图像数据一致。
- 修复写入复用常驻 ExifTool 进程，消除逐文件启动成本；仍逐文件执行备份、写入、验证和失败恢复。
- 常驻写入支持多行备注，读取每条命令的退出状态，保留已有备注与审计标记。

本机 Ryzen 7 9700X 样本实测（含进程启动，单次运行，文件缓存与磁盘会影响结果）：

| 操作 | v2.0.0 | v2.1.0 |
| --- | ---: | ---: |
| XMP 读取（128 个） | 96 个/秒 | 178 个/秒 |
| JPG 读取（63 个） | 16 个/秒 | 58 个/秒 |
| XMP 完整修复（24 个副本） | 2.4 个/秒 | 15.1 个/秒 |
| JPG 完整修复（24 个副本） | 2.1 个/秒 | 6.1 个/秒 |

读取时单个 ExifTool 约占一个 CPU 核；多个进程能利用多核。JPG 还受图像数据读取、备份和重写的 I/O 成本影响。GPS 计算占比较小。

可复现实测（只读取样本；修复发生在自动清理的临时副本上）：

```powershell
$env:LRTZ_BENCH_ROOT = 'D:\照片样本'
go test -tags "desktop,production" -run '^$' -bench BenchmarkExifToolPipeline -benchtime=1x -count=1
```
...
- 更新青蓝色视觉、操作步骤、空状态和摘要快捷筛选；替换为镜头时钟主题矢量图标，同步 Windows EXE 图标。
- 照片列表采用每页 100 条分页，搜索防抖，缩略图最多同时请求 2 张，丢弃过期请求，改善大批照片的浏览响应。
- 明确本页全选与跨页保留的关系，显示页外已选数量，支持清除全部选择。
- 增加 ExifTool 缺失的常驻提示与重新检测，完善忙碌状态、抽屉键盘操作和进度显示。
- 扫描进度覆盖元数据读取与 GPS 时区分析两个阶段；限制进度事件频率，复用已加载的时区规则，预分配结果列表。
- 加强退出时的扫描取消与缩略图会话清理；缩略图读取会话异常后允许重新创建。
