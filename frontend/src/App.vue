  ...
  <div class="app-shell">
    ...
    <div class="layout">
      ...
      <main class="workspace">
        ...
        <section v-if="busy === 'scan' || busy === 'repair'" class="progress-panel" role="status"><div><strong>{{ busy === 'scan' ? '正在扫描' : '正在修复' }}</strong><span>{{ progress.message }}</span></div><progress :value="progress.total ? percent : undefined" max="100"></progress><span>{{ progress.total ? `${percent}%` : '准备中' }}</span><button v-if="busy === 'scan'" id="cancelScanButton" class="button secondary" :disabled="cancelled" @click="cancelScan">{{ cancelled ? '正在终止…' : '终止扫描' }}</button></section>

        <template v-if="report">
          <div class="section-heading"><div><div class="eyebrow">分析结果</div><h2>检查完成，查看修复建议</h2></div><span class="subtle">{{ summary.total }} 个文件 · 扫描只读</span></div>
          ...
          <section v-if="hasXMP" class="xmp-note"><b>XMP 源头修正</b><span>无需 RAW 文件。修复后，在 Lightroom 中执行“从文件读取元数据”，让目录同步正确的拍摄时间。</span></section>
